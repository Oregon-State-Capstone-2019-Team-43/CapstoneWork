# Program for running the extract_ts_post_joke.praat script or
# the extract_ts_mid_joke.praat script on multiple performance directories
...
if type == "mid":
   ...
   for dir in subdirs:
      ...
      cmd = 'praat --run extract_ts_mid_joke.praat "'+fullpath+'" > '+filename
      os.system(cmd)
      mv = 'move '+filename+' ../mid_joke_praat_data/pitch_inten_ts > nul'
      os.system(mv)
      print("Data saved in %s\\mid_joke_praat_data\\pitch_inten_ts\\%s" % (basedir[:-20], filename))
      count = count+1
if type == "post":
   count = 1
   for dir in subdirs:
      ...
      print("\nWorking on files in %s" % fullpath)
      filename = str(count)+".txt"
      cmd = 'praat --run extract_ts_post_joke.praat "'+fullpath+'" > '+filename
      os.system(cmd)
